...
public class ServiceRequestValidator {
    ...
    private void validateAttributeValuesAgainstServiceDefinitionforUpdate(ServiceDefinition serviceDefinition, Service service) {
    ...
    private List<Service>  validateServiceid(String tenantId,String id) {
        List<org.egov.servicerequest.web.models.Service> listOfServices = serviceRequestRepository.getService(ServiceSearchRequest.builder().serviceCriteria(ServiceCriteria.builder().tenantId(tenantId).ids(Arrays.asList(id)).build()).build());

        if(listOfServices.isEmpty())
            throw new CustomException(SERVICE_REQUEST_EXISTS_ERR_CODE, SERVICE_REQUEST_EXISTS_ERR_CODE_MSG);

        return listOfServices;
    }

    public List<Service> validateServiceUpdateRequest(ServiceRequest serviceRequest) {
        if(Objects.isNull(serviceRequest.getService().getId())) {
            throw new CustomException(SERVICE_REQUEST_INVALID_SERVICE_ID_CODE, SERVICE_REQUEST_INVALID_SERVICE_ID_MSG);
        }
        String tenantId = serviceRequest.getService().getTenantId();
        String id = serviceRequest.getService().getId();
        String serviceDefId = serviceRequest.getService().getServiceDefId();

        List<Service> existingServices = validateServiceid(tenantId,id);

        if(!existingServices.get(0).getServiceDefId().equals(serviceDefId)) {
            throw new CustomException(SERVICE_REQUEST_INVALID_SERVICE_DEF_ID_CODE, SERVICE_REQUEST_INVALID_SERVICE_DEF_ID_MSG);
        }

        List<ServiceDefinition> serviceDefinitions = validateServiceDefID(serviceRequest.getService().getTenantId(), serviceRequest.getService().getServiceDefId());
        validateAttributeValuesAgainstServiceDefinitionforUpdate(serviceDefinitions.get(0), serviceRequest.getService());
